...
namespace VVPosM1.Screen
{
    public partial class frmManageProduct : Form
    {
        ...
        private void LoadProductType()
        {
            ...
            cboProductType.DataSource = new BindingSource(test, null);
        ...
        private void bntSeach_Click(object sender, EventArgs e)
        {
            ...
            proSea.ProductType = cboProductType.SelectedValue.ToString();
        ...
        private void SearchProducts(int page)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    dgv.Rows[i].Cells[2].Value = dr["Name"].ToString();
                    dgv.Rows[i].Cells[3].Value = string.Format("{0:0,0}", dr["Price"]);
                    
                    dgv.Rows[i].Cells[4].Value = dr["Unit"].ToString();
                    
                    string str = "";
                    DataTable dtP1 = proBLL.GetProductTypeId(dr["ProductType"].ToString());
                    ...
                    //Ghi chú
                    str = "";
                    str = str + dr["Notes"].ToString();
                    dgv.Rows[i].Cells[6].Value = str;


                    //Thông tin khởi tạo
                    str = "";
                    str = str + clsLanguages.GetResource("Account08") + " " + dr["CreatedBy"].ToString() + "\n";
                    str = str + clsLanguages.GetResource("Account09") + " " + dr["CreatedDate"].ToString() + "\n";
                    str = str + clsLanguages.GetResource("Account10") + " " + dr["ModifiedBy"].ToString() + "\n";
                    str = str + clsLanguages.GetResource("Account11") + " " + dr["ModifiedDate"].ToString();
        ...
        private void SaveData()
        {
            ...
            proSea.Notes = rtxtDescription.Text;
            proSea.Image = txtImage.Text.Substring(txtImage.Text.LastIndexOf("\\") + 1);
            proSea.ProductType = cboProductType.SelectedValue.ToString();
            ...
            {
                SaveImage(ProductID, proSea.Image);
            }
        ...
        private void SaveImage(string objID, string Image)
        {
            try
            {
                ...
                int i = proBLL.SaveImageIntoWeb(img, objID, Image);
                if (i == 1)
                {
                    txtImage.Text = "";
                    pbImgProduct.Image = null;
                }
            }
            catch (Exception e)
            {
        ...
        private void dgv_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                if (ob != null)
                {
                    ...
                    pbImgProduct.Image = VVPosM1.Common.Utility.GetImageFromService("Products", ob.Image);
        ...
        {
            Common.Utility.TextLengthValid(txtPrice, 26);
            Common.Utility.TextFormatNumber(txtPrice, ConfigurationManager.AppSettings["TextFormatNum"]);
        }
    }
}
