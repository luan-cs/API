    ...
    public class ProductsBLL : IObjectBLL<Products>
    {
        ...
        public DataTable SearchProducts(Products pro, int page, out int TotalRecord)
        {
            ...
            string[][] param ={
                ...
                new string[] { "p_ProductCode", string.IsNullOrEmpty(pro.productCode)?null:pro.ProductId },
                new string[] { "p_Name", string.IsNullOrEmpty(pro.Name)?null: pro.Name },
                new string[] { "p_Unit", string.IsNullOrEmpty(pro.Unit )?null: pro.Unit },
                new string[] { "p_Notes", string.IsNullOrEmpty(pro.Notes )?null: pro.Notes },
                new string[] { "p_ProductType", string.IsNullOrEmpty(pro.ProductType)?null: pro.ProductType },
                new string[] { "p_CreateBy", string.IsNullOrEmpty(pro.CreateBy)?null: pro.CreateBy },
                new string[] { "p_CreatedDate", string.IsNullOrEmpty(pro.CreatedDate)?null:pro.CreatedDate },
                new string[] { "p_ModifiedBy", string.IsNullOrEmpty(pro.ModifiedBy)?null: pro.ModifiedBy},
                new string[] { "p_NoStart", limit.ToString() },
                new string[] { "p_NoEnd", RecordPerPage.ToString()}
            };

            Program.destopService.DataStoreProcQuery_Param(Program.Username, Program.Password, "spSearch_Products", ref ds, param, ref errorString);
        ...
        public DataTable GetProductTypeId(string id)
        {
            ...
            string query = @"SELECT *
                            FROM `commoncode`
                            WHERE `CommonTypeId`='ProductType' and `CommonId`='" + id + "'";
            //Program.destopService.DataQuery(Program.Username, Program.Password, query, ref ds, "X", ref errorString);
            Program.destopService.DataQuery(Program.Username, Program.Password, query, ref ds, "X", ref errorString);
            if (string.IsNullOrEmpty(errorString))
            {
        ...
        public Products GetProductWithID(string id, string name = "")
        {
            ...
            if (string.IsNullOrEmpty(errorString) && ds.Tables[0].Rows.Count > 0)
            {
                ...
                reEmp.Name = ds.Tables[0].Rows[0]["Name"].ToString();
                reEmp.Price = ds.Tables[0].Rows[0]["Price"].ToString();
                reEmp.Unit = ds.Tables[0].Rows[0]["Unit"].ToString();
        ...
        public int Update(Products employ)
        {
            ...
            if (!string.IsNullOrEmpty(employ.Name))
            {
                queryStr += string.Format(" Name = '{0}',", employ.Name);
            }
            //if (!string.IsNullOrEmpty(employ.Unit))
            //{
            queryStr += string.Format(" Unit = '{0}',", employ.Unit);
            //}
            //if (!string.IsNullOrEmpty(employ.Notes))
            //{
            queryStr += string.Format(" Notes = '{0}',", employ.Notes);
            //}
            queryStr += string.Format("Price = '{0}',", double.Parse(employ.Price).ToString());

            queryStr += string.Format("Image = '{0}',", employ.Image);

            if (!string.IsNullOrEmpty(employ.ProductType))
            {
                queryStr += string.Format(" ProductType = '{0}',", employ.ProductType);
            }
            queryStr += string.Format(" ModifiedBy = '{0}',", Program.Username);
            queryStr += " ModifiedDate = NOW()";
            queryStr += string.Format(" WHERE ProductId = '{0}'", employ.ProductId);
        ...
        public int Insert(Products employ)
        {
            ...
            if (!string.IsNullOrEmpty(employ.Image))
            {
                queryStr += "`Image`,";
            }
            ...
            if (!string.IsNullOrEmpty(employ.Name))
            {
                queryStr += string.Format("'{0}',", employ.Name);
            }
            if (!string.IsNullOrEmpty(employ.Price))
            {
                queryStr += string.Format("'{0}',", double.Parse(employ.Price).ToString());
            }
            if (!string.IsNullOrEmpty(employ.Unit))
            {
                queryStr += string.Format("'{0}',", employ.Unit);
            }
            if (!string.IsNullOrEmpty(employ.Notes))
            {
            ...
            if (!string.IsNullOrEmpty(employ.Image))
            {
                queryStr += string.Format("'{0}',", employ.Image);
            }
            if (!string.IsNullOrEmpty(employ.ProductType))
            {
                queryStr += string.Format("'{0}',", employ.ProductType);
            }
        ...
        public int Delete(string id)
        {
            ...
            /// 0: xóa ko thành công
            int res = 0;
            string queryStr = string.Format("DELETE FROM products WHERE Productid = '{0}'", id);
            Program.destopService.DataExecute(Program.Username, Program.Password, queryStr, ref errorString);
            if (string.IsNullOrEmpty(errorString))
            {
                res = 1;
