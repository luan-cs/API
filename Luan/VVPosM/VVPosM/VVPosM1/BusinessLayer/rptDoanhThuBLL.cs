...
//        }
        ...
        {string dateStart= tuNgay;
            ...
            if (dt1 != null)
            {
                ...
                {
                    fullName = objectBLL.GetObjectByObjectId(dr["ObjectId"].ToString()).Rows[0]["FullName"].ToString();
                    DataRow dra = dt.NewRow();
                    dra["STT"] = (i + 1).ToString();
                    dra["FullName"] = Common.clsLanguages.GetResource("EmployeeCode") + ": " + dr["UserId"].ToString() + "\n" + Common.clsLanguages.GetResource("EmployeeName") + ": " + fullName;
        ...
        public DataTable GetReport(string userID,string tuNgay, string denNgay, int check)
        {
            ...
            DataTable res= new DataTable();
            DataSet ds = new DataSet();
            string query = @" SELECT b.`ObjectId`,b.`UserId`,SUM(a.`TotalMoney`) AS `DoanhThuCuoi`, SUM(a.`TotalAmountBeforeTax`) AS `DoanhThuTruocThue`, SUM(a.`TotalTax`) AS `TongTienThue`, SUM(a.`DiscountAmount`) AS `TongTienGiamGia`, SUM(a.`TotalAmount`) AS `TongTien`
                         FROM `receipts` a                         
                         INNER JOIN `users` b
