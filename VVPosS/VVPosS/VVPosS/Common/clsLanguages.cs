...
namespace VVPosS.Common
{
    class clsLanguages
    {
        ...
        public static string GetResource(string skey)
        {
            ...
            DataTableCollection dtc = Program.LangRes;
            Hashtable HasTableLang = new Hashtable();
            try
            {
                int LangId = GetLangIdBySession(Common.clsLanguages.StrCulture);
                //if (LangId == 5)
                //{
                //    LangId = 1;
                //}
        ...
        public static DataTableCollection ReadExcelFromURL(string FullFileName)
        {
            try
            {
                ...
                //IExcelDataReader excelReader = ExcelReaderFactory.CreateOpenXmlReader(data);
